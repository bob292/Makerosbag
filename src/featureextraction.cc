...
void FeatureExtraction::Extract(FullCloudPtr pc_in, CloudPtr pc_out_edge, CloudPtr pc_out_surf, std::vector<std::vector<int>> &edge_pixel_index) {
    int num_scans = 1345;
    ...
    for (int i = 0; i < num_scans; i++) {                                                                                                             
        ...
        std::vector<int> edge_row_index;
        // 对每个区间选取特征，把360度分为6个区间
        const int num_section = 1;
        for (int j = 0; j < num_section; j++) {
            ...
            std::vector<int> sub_edge_row_index;
            ExtractFromSector(scans_in_each_line[i], sub_cloud_curvature, pc_out_edge, pc_out_surf, edge_row_index);
        }
        edge_pixel_index.push_back(edge_row_index);
    }
}

void FeatureExtraction::ExtractFromSector(const CloudPtr &pc_in, std::vector<IdAndValue> &cloud_curvature,
                                          CloudPtr &pc_out_edge, CloudPtr &pc_out_surf, std::vector<int> &edge_row_index) {
    // 按曲率排序
    ...
    for (int i = cloud_curvature.size() - 1; i >= 0; i--) {
        ...
        if (std::find(picked_points.begin(), picked_points.end(), ind) == picked_points.end()) {
            ...
            // filtered points whose depth > 40
            if (largest_picked_num <= 20 && pc_in->points[ind].z <= 40.) {//&& pc_in->points[ind].z <= 40.) {
                pc_in->points[ind].intensity = 5;
                pc_out_edge->push_back(pc_in->points[ind]);
                edge_row_index.push_back(ind);
                point_info_count++;
            } else {
                break;
            }
    ...
    for (int i = 0; i <= (int)cloud_curvature.size() - 1; i++) {
        int ind = cloud_curvature[i].id_;
        if (std::find(picked_points.begin(), picked_points.end(), ind) == picked_points.end() && pc_in->points[ind].z <= 60.) {
